...
const AvailablePhones = () => {
  ...
  const phonesList = DUMMY_PHONES.slice(0, shownItemsNumber).map((phone) => (
    <PhonesItem
      id={phone.id}
      key={phone.id}
      title={phone.title}
  ...
  return (
    <section className={classes.phones}>
      <Card>
        <ul>{phonesList}</ul>
        {shownItemsNumber < DUMMY_PHONES.length && (
          <button className={classes.button} onClick={showMoreItemsHandler}>View More</button>
        )}
      </Card>
    </section>
  );
